...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  public int indexOf(Position<E> p) {
    Position<E> currentPosition = first();  // Start from the first position
    int index = 0;  // Initialize the index

    // Traverse the list until encountering position p
    while (currentPosition != null) {
      if (currentPosition.equals(p)) {
        return index;  // Return the index if position p is found
      }
      currentPosition = after(currentPosition);  // Move to the next position
      index++;  // Increment the index
  }
    throw new IllegalArgumentException("Position p is not in the list");
  }

  public static void main(String[] args) {

    LinkedPositionalList<String> linkedList = new LinkedPositionalList<>();
    ...
    for(String OUT : linkedList)
    {
      System.out.println(OUT);
    }
    Position<String> positionP = linkedList.header.getNext().getNext().getNext(); //Input for Testing for index of 2

    try {
      // Test the indexOf(p) method
      int indexOfP = linkedList.indexOf(positionP);
      System.out.println("Index of position p: " + indexOfP);
